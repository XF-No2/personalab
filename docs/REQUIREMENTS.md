...
**定义**：一个独立的对话，包含完整的对话历史和角色成长状态。

**本质**：对话文件是核心数据，预设（角色、背景、摘要）是可复用的Prompt模块。用户可以创建多个对话，每个对话的角色状态、剧情进度、历史事件完全独立。

**创建方式**：
- 用户选择一个角色预设和背景预设 → 创建新对话
- 系统初始化：
  - 创建 conversation.jsonl（存储对话历史）
  - 创建 metadata.json（记录预设引用和RAG配置）
  - 创建 character_snapshot.json（从角色预设复制快照，后续独立演化）
  - 创建 plot_state.json（可选，如果背景包含故事大纲）

**预设引用**：
- 角色预设：角色的初始人格设定（可复用）
- 背景预设：世界设定和故事大纲（可复用）
- 摘要预设：其他对话生成的摘要（可选，用于引用历史经验）
- 当前使用的预设引用记录在 metadata.json 中

**核心特性**：
1. **状态隔离**：同一角色在不同对话中的状态完全独立
2. **可重玩**：同样的角色+背景可以创建多个对话，体验不同走向
3. **灵活组合**：对话文件可以随时打开，预设可以自由选择
4. **快照机制**：创建对话时从预设复制快照，后续独立演化

**示例**：
```
对话A：角色预设"alserqi" + 背景预设"wasteland"
→ 第3点选择"杀死仇人" → 走向暴力结局

对话B：同一角色预设 + 同一背景预设（重玩）
→ 第3点选择"放过仇人" → 走向救赎结局
...
### 对话管理
- 创建对话：选择角色预设和背景预设（背景可选）
- 查看对话列表：显示标题、使用的预设、创建时间、最后对话时间
- 继续对话：加载对话历史和角色快照，继续对话
- 删除对话
...
9. **角色编辑**（用户端）
   - 编辑预设库中的角色预设（presets/characters/*.json）
   - 这是"角色管理"的基础功能（CRUD 的 U）
   - 注意：不能编辑对话中的角色快照（那个属于对话）
...
### v4.0 (2025-10-17 00:50)
- **架构适配**：从"会话实例"架构更新为"对话文件+预设引用"架构
- **术语更新**：
  - 会话实例 → 对话（Conversation）
  - instance_state.json → metadata.json
  - character_state.json → character_snapshot.json
...
- **数据结构更新**：
  ...
  - metadata.json：预设引用和RAG配置
  - character_snapshot.json：角色快照
  - plot_state.json：剧情进度（可选）

### v3.0 (2025-10-16)
- 精简设计细节，只保留需求本身
- 删除设计方案，移至DESIGN_OVERVIEW.md
- 新增需求追溯链接
