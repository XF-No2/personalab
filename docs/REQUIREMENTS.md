...
导演模块负责控制AI按照故事大纲推进剧情，核心子功能是**剧情大纲控制**。
...
#### evolved_persona（成长人格）
- **内容**：角色经历成长后的状态
- **包含**：信念变化、行为模式、关系深度、当前情绪等
- **位置**：Prompt头部4K区域（紧随base_persona）
- **维护**：用户手动触发"🧠 更新记忆"按钮
  - 使用LLM整合"base_persona + 旧evolved_persona + 最近对话"
  - 重新生成完整的evolved_persona
- **格式**：纯定性描述，不使用量化数值
...
#### 事件库（Event Library）
- **内容**：历史事件摘要（汇总功能生成）
- **存储**：向量数据库（Chroma，分层摘要）
- **位置**：Prompt 32K+区域（RAG检索后注入）
- **写入时机**：用户触发汇总功能时写入
...
**应对**：使用向量数据库RAG检索历史事件，角色状态（base_persona + evolved_persona）存储当前人格。
...
**应对**：evolved_persona中的人格描述控制在该复杂度范围内。
...
通过三个部分协同工作：
1. **情节点设置**：全量主线大纲放头部4K，JSON格式，标记当前进度
2. **进度引导**：AI每次回复输出`[PROGRESS:X:status]`，后端正则扫描更新状态
3. **容错处理**：连续3轮未更新时，用当前大纲点查询RAG，注入相关事件拉回主线

**效果**：像一根绳子，允许AI在细节上自由发挥，但拉住AI不要偏离主线方向

**扩展性**：导演模块是框架，未来可扩展更多子功能
...
**决策**：采用经典的三栏布局，类似 SillyTavern 和其他 LLM 对话界面
