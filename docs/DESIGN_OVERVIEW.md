...
│   - 内容：父子结构的情节数据
│       ├─ 父节点（Summaries Collection）：粗粒度摘要
│       └─ 子节点（Plots Collection）：细粒度剧情素材
...
[4] 写入 Chroma（父子结构）
  for each 情节点:
    ├─ 父节点 → Summaries Collection
    │   - ID: summary_{session_id}_{index}
    │   - 内容: summary（摘要）
    │   - 元数据: child_id, session_id
    │
    └─ 子节点 → Plots Collection
...
[6] 创建新会话
  - 初始化内容（可配置顺序）:
      所有父节点摘要 + 最后5轮对话
  - 注：只展示父节点（简洁），子节点保存在库中
  ↓
[7] 返回前端，自动切换到新会话
```

**关键设计**：
- **父子结构**：一次 LLM 调用生成两层数据（摘要 + 详细素材）
- **严格解析**：JSON 格式，解析失败直接报错，由用户决定重试
- **分层存储**：父节点和子节点分别存入不同 Collection
- **灵活展示**：新会话只展示摘要（简洁），详细素材保存在库中供 RAG 检索
- **可复用素材**：详细素材可以被导演模块、剧情设计等功能复用
...
用户输入："你还记得之前的复仇吗？"
  ↓
RAG 搜索 Summaries Collection（只查父节点）
  ↓
返回相关摘要：
...
找到相关父节点：summary_001
  ↓
通过 child_id 查询子节点（Plots Collection）
  ↓
返回：
  - 父节点：summary_001（摘要）
  - 子节点：plot_001（详细素材："小张假装和解，邀请小玲到废弃工厂..."）
  ↓
组装 Prompt（包含摘要 + 详细素材）
```
- 适用：需要具体细节、情节设计参考
...
3. ✅ **取消滑动窗口机制** → 不限制对话数量
4. ✅ **汇总功能**：独立 LLM Agent，一次调用生成父子结构
5. ✅ **父子结构存储**：
   - 父节点（摘要）：粗粒度，用于延续历史
   - 子节点（详细素材）：细粒度，可复用的剧情设计
   - Chroma 分两个 Collection 存储
6. ✅ **RAG 深度搜索**：支持浅层（只查父节点）和深度（展开子节点）
