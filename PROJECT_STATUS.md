...
2. **Chroma 分层存储**
   - Summaries Collection：父节点，存储摘要
   - Plots Collection：子节点，存储详细素材
   - 通过 parent_id / child_id 建立关联

3. **RAG 深度搜索策略**
   - 浅层搜索：只查父节点（摘要），用于日常对话
   - 深度搜索：展开子节点（详细素材），用于需要具体细节时
...
**文档更新**：
- DESIGN_OVERVIEW.md v0.3.0 → v0.3.1
